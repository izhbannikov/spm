...
//Calculating m (y[1]) & gamma(y[2]):
void func1_linear(arma::mat *res, double t, arma::mat *y, arma::mat fH, arma::mat f1H, arma::mat aH, 
           arma::mat bH, arma::mat QH, double theta, arma::mat QH1) {
...
RcppExport SEXP complikMD_linear(SEXP dat, 
    ...
    for(int i=0; i<N; i++) {
        ...
        //Integration:
        gamma1.zeros(); // set gamma1 to zero matrix
        double s = h/3.00*(-1.00)*mu_linear(t1, y1, gamma1, fH, f1H, mu0H, thetaH, QH, Q1);
        double t = t1;
        out[0] = y1;
        out[1] = gamma1;
        double ifactor;
        ...
        for(int j = 0; j < nsteps; j++) {
            //Runge-Kutta method:
            func1_linear(k1ar, t, out, fH, f1H, aH, bH, QH, thetaH, Q1);
            yfin[0] = out[0] + h/6.00*k1ar[0];
            yfin[1] = out[1] + h/6.00*k1ar[1];
            ytmp[0] = out[0] + h/2.00*k1ar[0];
            ytmp[1] = out[1] + h/2.00*k1ar[1];
      
            func1_linear(k2ar, t, ytmp, fH, f1H, aH, bH, QH, thetaH, Q1);
            yfin[0] = yfin[0] + h/3.00*k2ar[0];
            yfin[1] = yfin[1] + h/3.00*k2ar[1];
            ytmp[0] = out[0] + h/2.00*k2ar[0];
            ytmp[1] = out[1] + h/2.00*k2ar[1];
      
            func1_linear(k3ar, t, ytmp, fH, f1H, aH, bH, QH, thetaH, Q1);
            yfin[0] = yfin[0] + h/3.00*k3ar[0];
            yfin[1] = yfin[1] + h/3.00*k3ar[1];
            ytmp[0] = out[0] + h*k3ar[0];
            ytmp[1] = out[1] + h*k3ar[1];
      
            func1_linear(k4ar, t, ytmp, fH, f1H, aH, bH, QH, thetaH, Q1);
            out[0] = yfin[0] + h/6.00*k4ar[0];
            out[1] = yfin[1] + h/6.00*k4ar[1];
        ...
        if(dd(i,0) == 0) { 
            ...
            //double logprobi = log(1.00 - exp(-1.00*mu(t2, m2, gamma2, fH, f1H, mu0H, thetaH, QH)));
            double logprobi = log(mu_linear(t2, m2, gamma2, fH, f1H, mu0H, thetaH, QH, Q1));
            L += s + logprobi;
        }
    }
