...
spm_integral_MD <- function(dat,parameters) {
  ...
  setBoundaries <- function() {
    ...
    lower_bound <- c(lower_bound, unlist(lapply(start:end, function(n) {x=ifelse(pars_prev[n] >= 0, 1e-9, -1e-7)})) )
    upper_bound <- c(upper_bound, unlist(lapply(start:end, function(n) {x=ifelse(pars_prev[n] >= 0, 1e-7, -1e-9)})))
    # fH
    start=end+1
    end=start+kk-1
    lower_bound <- c(lower_bound, unlist(lapply(start:end, function(n){pars_prev[n] + ifelse(pars_prev[n] <= 0, 0.25*pars_prev[n], -0.25*pars_prev[n]) })) )
